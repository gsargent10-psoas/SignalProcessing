...
double min_double(double* img, int size)
{
...
float min_float(float* img, int size)
{
	float current=img[0];
	float min = current;
	if (size == 1)
...
double max_double(double* img, int size)
{
	double current=img[0];
	double max = current;
	if (size == 1)
...
float max_float(float* img, int size)
{
	float current=img[0];
	float max = current;
	if (size == 1)
...
double mean_double(double* img, int size)
{
	float total = 0.0;
...
float mean_float(float* img, int size)
{
	float total = 0.0;
	for(int k=0;k<size;k++)
	{
...
double std_double(double* img, int size)
{
	double mu = mean_double(img,size);
	double sum = 0.0;
	for(int k=0;k<size;k++)
	{
...
float std_float(float* img, int size)
{
	float mu = mean_float(img,size);
	float sum = 0.0;
	for(int k=0;k<size;k++)
	{
		sum = sum + pow(fabs((img[k]-mu)),2);
...
int imgscale_double(double* img, int size, double param1, double param2, int type)
{
	double alpha_l, alpha_h, mu, sd;
	if (type==0)
	{
		alpha_l = min_double(img,size);
		alpha_h = max_double(img,size);
	}
	else if (type==1)
	{
		mu = mean_double(img,size);
		sd = std_double(img,size);
...
int imgscale_float(float* img, int size, double param1, double param2, int type)
{
	double alpha_l, alpha_h, mu, sd;
	if (type==0)
	{
		alpha_l = min_float(img,size);
		alpha_h = max_float(img,size);
	}
	else if (type==1)
	{
		mu = mean_float(img,size);
		sd = std_float(img,size);
		alpha_l = mu - param1*sd;
		alpha_h = mu + param2*sd;
	}
	else if (type==2)
...
int kmeans_double(double* in, int* out, int size, int k, int iterations)
{
...
int kmeans_float(float* in, int* out, int size, int k, int iterations)
{
	// Step-1: Select value of k, to decide number of clusters to be formed.
	if (k < 1 || k > size/2)
	{
