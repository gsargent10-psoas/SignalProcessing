...
class Polarimetric:
    def __init__(self,lib_path='./libpolarimetric.so'):
        ...
        self._getNumberSubImages = self.lib.getNumberSubImages
        self._stitchsubimages_double = self.lib.stitchsubimages_double
        self._stitchsubimages_float = self.lib.stitchsubimages_float
        self._formSubImage_double = self.lib.formSubImage_double
        self._formSubImage_float = self.lib.formSubImage_float
        ...
        self._getNumberSubImages.restype = ctypes.c_int
        self._getNumberSubImages.argtypes = [ctypes.c_int,ctypes.c_int,ctypes.c_int, ctypes.c_int,ctypes.c_int]
        ...
        #int formSubImage(double* image, int image_rows, int image_cols, double* sub_images, int sub_rows, int sub_cols, int overlap, int num_sub);
        self._formSubImage_double.restype = ctypes.c_int
        self._formSubImage_double.argtypes = [ndpointer(dtype=np.double, flags="C_Contiguous"),ctypes.c_int, ctypes.c_int, 
        ndpointer(dtype=np.double, ndim=3, flags="C_Contiguous"),ctypes.c_int,ctypes.c_int,ctypes.c_int, ctypes.c_int]
        self._formSubImage_float.restype = ctypes.c_int
        self._formSubImage_float.argtypes = [ndpointer(dtype=np.float32, flags="C_Contiguous"),ctypes.c_int, ctypes.c_int, 
        ndpointer(dtype=np.float32, flags="C_Contiguous"),ctypes.c_int,ctypes.c_int,ctypes.c_int, ctypes.c_int]
    ...
    #int formSubImage(double* image, int image_rows, int image_cols, double* sub_images, int sub_rows, int sub_cols, int overlap, int num_sub);
    def formSubImage(self,image:np.double,sub_image:np.double,overlap:int)->int:
        iy,ix=image.shape
        sy,sx,num=sub_image.shape
        return self._formSubImage_double(np.ascontiguousarray(image, dtype=np.double),iy,ix,
        np.ascontiguousarray(sub_image, dtype=np.double),sy,sx,overlap,num)
    def formSubImage(self,image:np.float32,sub_image:np.float32,overlap:int)->int:
        iy,ix=image.shape
        sy,sx,num=sub_image.shape
        return self._formSubImage_float(np.ascontiguousarray(image, dtype=np.float32),iy,ix,
        np.ascontiguousarray(sub_image, dtype=np.float32),sy,sx,overlap,num)
